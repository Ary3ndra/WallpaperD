...
import org.piarsenal.wallpaperd.data.PrefetchedImage
...
import org.piarsenal.wallpaperd.provider.ProviderEnv
import org.piarsenal.wallpaperd.provider.ProviderFactory
import java.io.File
    ...
    suspend fun prefetchNext(context: Context) = withContext(Dispatchers.IO) {
        mutex.withLock {
            ...
            for (cfg in sources.shuffled()) {
                ...
                org.piarsenal.wallpaperd.data.CacheManager.enforceLimit(context)
                return@withLock
            }
        }
    }
